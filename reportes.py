...
# Variables Globales de Rutas inicializadas para evitar errores de definición
ruta_mensual = ""
ruta_anual = ""

# Configuración de filas por hoja para impresión
FILAS_POR_PAGINA = 50 
...
# --- SISTEMA DE ACTUALIZACIÓN CON RELEVO SEGURO ---
...
def actualizar_programa():
    try:
        ...
        # Consultar la última versión en la API de GitHub
        ...
        # Script .bat con 20 segundos de espera para PCs lentas y limpieza de temporales
        with open("update_script.bat", "w") as f:
            f.write(f'@echo off\n')
            f.write(f'title Actualizando Sistema - Por favor espere...\n')
            f.write(f'echo Preparando reemplazo de archivos (20 segundos)...\n')
            f.write(f'timeout /t 20 /nobreak > nul\n') 
            f.write(f'del "{EXE_NAME}"\n')
            f.write(f'ren "{temp_exe}" "{EXE_NAME}"\n')
            f.write(f'start "" "{EXE_NAME}"\n')
            f.write(f'del "%~f0"\n')

        messagebox.showinfo("Éxito", "Descarga finalizada. El programa se cerrará y se actualizará en 20 segundos automáticamente.")
        ...
        # Intentamos rehabilitar el botón si el programa no se cerró
...
def generar_reportes():
    ...
    try: # <--- Bloque try ahora tiene su except y finally
    ...
    finally: # <--- Aseguramos que el botón se reactive siempre
